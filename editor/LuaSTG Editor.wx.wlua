...
resList={snd={}}
function LoadSound(name,path) resList.snd[name]=path end

dofile("..\\game\\thlib\\se\\se.lua")
...
function main()
	...
	for i=1,16 do
		paramNameLabel[i]=dialog.InputParameter:FindWindow(ID("ParamNameLabel"..i)):DynamicCast("wxStaticText")
		paramText[i]=dialog.InputParameter:FindWindow(ID("ParamText"..i)):DynamicCast("wxTextCtrl")
	end
	--
	...
	for i=1,34 do
		selectEnemyStyleButton[i]=dialog.SelectEnemyStyle:FindWindow(ID("Style"..i)):DynamicCast("wxBitmapButton")
	end
	--
	...
	for i,n in ipairs(enumType.bulletshow) do
		selectBulletStyleButton[i]=dialog.SelectBulletStyle:FindWindow(ID(n)):DynamicCast("wxBitmapButton")
	end
	--
	...
	for i,n in ipairs(enumType.color) do
		selectColorButton[i]=dialog.SelectColor:FindWindow(ID(n)):DynamicCast("wxBitmapButton")
	end
	--
	local picker={
	...
	local function LoadFromFile(file,ignore_setting)
		...
		if tmp._proj_version and tmp._proj_version>_luastg_version then return 'LuaSTG version is too low.' end
		for i=1,#tmp do
	...
	function CompileToFile(f,node,depth)
		...
		-- debug
		if debugNode and debugNode[2]==node:GetValue() then f:write('end ') end
		-- head
		...
		-- debug
		if debugNode and debugNode[1]==node:GetValue() then f:write('if false then ') end
	...
	local function PackProj(event)
		if curProjFile then
			...
			if debugCode then f:write(debugCode) end
			if scDebugNode then
	...
	local function ToolMove(mode)
		...
		if mode=='down' then insertPos='after' node0=projectTree:GetNextSibling(curNode)
			if not IsValid(node0) then
				node0=projectTree:GetItemParent(curNode)
			end
		end
	...
	--
	frame:Connect(ID"ToolMoveUp",wx.wxEVT_COMMAND_TOOL_CLICKED,function(event) ToolMove('up') end)
	frame:Connect(ID"ToolMoveDown",wx.wxEVT_COMMAND_TOOL_CLICKED,function(event) ToolMove('down') end)
	...
	local function EditAttr(event)
		...
		if data then
			...
			elseif enum=="resfile" then
				local wildCard
				if data.type=='loadsound' or data.type=='loadbgm' then wildCard="Audio file (*.wav;*.ogg)|*.wav;*.ogg"
				elseif data.type=='loadimage' or data.type=='loadani' or data.type=='bossdefine'  or data.type=='bossexdefine' then wildCard="Image file (*.png;*.jpg;*.bmp)|*.png;*.jpg;*.bmp"
				...
				elseif data.type=='loadFX' then wildCard="FX file (*.fx)|*.fx"
	...
	dialog.SoundEffect:Connect(wx.wxEVT_COMMAND_LISTBOX_SELECTED,function(event)
		...
		if resList.snd[sel] then wx.wxSound.Play("..\\game\\data\\"..resList.snd[sel])
		else
	...
	dialog.Type:Connect(wx.wxEVT_SHOW,function(event)
		if dialog.Type:IsShown() then
			...
			elseif t=='laserbentcreate' then watch='laserbentdefine'
			elseif t=='bossdefine' then watch='bgdefine'
			elseif t=='bossexdefine' then watch='bgdefine'
			elseif t=='reboundercreate' then watch='rebounder'
			elseif t=='taskattach' then watch='taskdefine'
			else
				--
			end
			local list={}
			local function append(s) if not list[s] then list[s]=true typeListBox:Append(s) end end
			if t~='bossdefine' and t~='bossexdefine' then
				for k,v in pairs(Tree.watch[watch]) do
					local tmp=string.match(Tree.data[k].attr[1],'^(.+):.+$')
					if tmp then append(tmp) else append(Tree.data[k].attr[1]) end
				end
			else
				for k,v in pairs(Tree.watch[watch]) do
	...
	local function FindNodeByTypeName(node,tname)
		if _def_node_type[node.type] then
			if node.attr[1]==tname then return node else return end
		else
			local ret
			for k,v in pairs(node.child) do
	...
	local function FindDifficulty(node)
		while true do
			if node:GetValue()==rootNode:GetValue() then break end
	...
	dialog.InputTypeName:Connect(wx.wxEVT_SHOW,function(event)
		if dialog.InputTypeName:IsShown() then
			...
			if t1 then typeNameText:SetValue(t1) difficultyCombo:SetValue(t2)
			else typeNameText:SetValue(tname) difficultyCombo:SetValue("All") end
		end
		frame:Enable(not frame:IsEnabled())
		event:Skip()
	end)
	...
	dialog.Image:Connect(wx.wxID_OK,wx.wxEVT_COMMAND_BUTTON_CLICKED,function(event)
		if imageListBox:GetStringSelection()~="" then attrCombo[attrIndex]:SetValue(imageListBox:GetStringSelection()..imageString:GetValue()) end
		SubmitAttr()
		event:Skip()
	end)
	...
	local function ImagePrevPanelPaint(event,x,y)
		...
		if imageList[sel] then
			...
			local scale0=imagew
			if imageh>imagew then scale0=imageh end
			scale0=scale0/256
	...
	dialog.Image:Connect(wx.wxEVT_SHOW,function(event)
		if dialog.Image:IsShown() then
			...
			if imgonly then
				for k,v in pairs(Tree.watch.image) do
					if Tree.data[k]["type"]=='loadimage' then
						if('image:'..Tree.data[k].attr[2]==current) then _find=true end
				...
				for k,v in pairs(Tree.watch.image) do
					if Tree.data[k]["type"]=='loadimage' then
						...
						imageList['image:'..Tree.data[k].attr[2]]=Tree.data[k].attr[1]
					elseif Tree.data[k]["type"]=='loadani' then
						if('ani:'..Tree.data[k].attr[2]==current) then _find=true end
						imageListBox:Append('ani:'..Tree.data[k].attr[2])
						imageList['ani:'..Tree.data[k].attr[2]]=Tree.data[k].attr[1]
					elseif Tree.data[k]["type"]=='loadparticle' then
						if('particle:'..Tree.data[k].attr[2]==current) then _find=true end
