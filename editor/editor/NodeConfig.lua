...
function toboolean(v)
	if type(v)=="string" then
		if v=="false" or v=="nil" or not(string.match(v,"^[%s]*$")) then
			return false
		else
			return true
		end
...
nodeType={
	['setting']={
		...
		check=function(nodedata)
			...
			if attr[5]=='lib' then projectmode='lib' else projectmode='game' end
		end,
	...
	listcreate={
		...
		forbidparent={'root','folder'},
		totext=function(nodedata)
			if IsBlank(nodedata.attr[2]) then return 'define local list ('..nodedata.attr[1]..')'
			else return 'define local list ('..nodedata.attr[1]..") = "..nodedata.attr[2] end
		end,
		tohead=function(nodedata)
			if IsBlank(nodedata.attr[2]) then return 'local '..nodedata.attr[1].."={}\n"
			else return 'local '..nodedata.attr[1].." = "..nodedata.attr[2].."\n" end
	...
	listadd={
		...
		totext=function(nodedata) return string.format("Add %s to list (%s)",nodedata.attr[2],nodedata.attr[1]) end,
		tohead=function(nodedata)
		...
		check=function(nodedata)
			...
			for i=2,8,2 do
				if not IsBlank(attr[i]) then
					...
					return string.format("Properties '%s' is not set:",attr[i]) end
				end
			end
		end,
		totext=function(nodedata)
	...
	stagegroup={
		...
		tohead=function(nodedata) stageGroupName=nodedata.attr[1] return string.format("stage.group.New('menu',{},%q,{lifeleft=%s,power=%s,faith=%s,bomb=%s},%s,%s)\n",
			nodedata.attr[1],nodedata.attr[2],nodedata.attr[3],nodedata.attr[4],nodedata.attr[5],nodedata.attr[6],nodedata.attr[7]) end,
		check=function(nodedata) difficulty=nodedata.attr[1] end,
	...
	stagegroupinfo={
		...
		totext=function(nodedata) return string.format("players %s~%s",nodedata.attr[1],nodedata.attr[2]) end,
		tohead=function(nodedata) return string.format("stage.groups['%s'].info={players={%s,%s}}\n",stageGroupName,nodedata.attr[1],nodedata.attr[2]) end,
	},
	stage={
		{'Name','any',CheckName},
	...
	stagegoto={
		...
		tohead=function(nodedata) return
			"New(mask_fader,'close')\
			task.New(self,function()\
				local _,bgm=EnumRes('bgm')\
				for i=1,30 do \
	...
	-- task
	task={
		needancestor={'enemydefine','bossdefine','objectdefine','laserdefine','laserbentdefine',
		'bulletdefine','rebounderdefine','enemysimple','renderimage','task','taskdefine','stagetask','tasker','bglayer','bossspellcardex','bossexdefine'},
		totext=function(nodedata) return "create task" end,
		tohead=function(nodedata) return "lasttask=task.New(self,function()\n" end,
		tofoot=function(nodedata) return "end)\n" end,
	},
	tasker={
		forbidparent={'root','folder'},
		totext=function(nodedata) return "create tasker" end,
		tohead=function(nodedata) return "New(tasker, function()\n" end,
		tofoot=function(nodedata) return "end)\n" end,
	},
	taskclear={
		{'keep this','bool'},
	...
	taskwait={
		...
		needancestor={'task','taskdefine','stagetask','dialogtask','tasker'},
		allowchild={},
		totext=function(nodedata)
	...
	taskwaitto={
		...
		totext=function(nodedata)
			...
			if nodedata.attr[2]=='UNIT_FRAME' then z=' frame(s)' end
			if nodedata.attr[2]=='UNIT_MUSIC' then z=' pace(s)' end
	...
	signal={
		...
		tohead=function(nodedata) return "ex.SetSignal('"..nodedata.attr[1].."',"..nodedata.attr[2]..")\n" end,
	},
	taskrepeat={
		{'Number of times','any',CheckExpr},
	...
	taskreturn={
		needancestor={'task','taskdefine','dialogtask','tasker'},
		disptype='terminate task',
		allowchild={},
		totext=function(nodedata) return "terminate current task" end,
		tohead=function(nodedata) return "do return end\n" end,
	},
	taskmoveto={
	...
	tasksetvalue={
		...
		totext=function(nodedata)
			...
			local nt
			if IsBlank(nodedata.attr[4]) then nt='1' else nt=nodedata.attr[4] end
			local action=''
		...
		tohead=function(nodedata)
			...
			if IsBlank(nodedata.attr[1]) then getter='function () return '..nodedata.attr[2]..' end' else getter='"'..nodedata.attr[2]..'"' end
			if IsBlank(nodedata.attr[1]) then setter='function (__) '..nodedata.attr[2]..'=__ end' else setter='nil' end
			if IsBlank(nodedata.attr[1]) then binder='self' else binder=nodedata.attr[1] end
			
			if IsBlank(nodedata.attr[4]) then nf='1' else nf=nodedata.attr[4] end
			if IsBlank(nodedata.attr[5]) then mode='MOVE_NORMAL' else mode=nodedata.attr[5] end
			if IsBlank(nodedata.attr[6]) then start='0' else start=nodedata.attr[6] end
			if IsBlank(nodedata.attr[7]) then vmode='MODE_SET' else vmode=nodedata.attr[7] end
	...
	taskBeziermoveto={
		...
		totext=function(nodedata)
			...
			for i=4,7 do
				if IsBlank(nodedata.attr[i]) then break
				else str=str..'),('..nodedata.attr[i] end
			end
		...
		tohead=function(nodedata)
			local nf,mode
			if IsBlank(nodedata.attr[1]) then nf='1' else nf=nodedata.attr[1] end
			if IsBlank(nodedata.attr[2]) then mode='MOVE_NORMAL' else mode=nodedata.attr[2] end
			local str=nodedata.attr[3]
			for i=4,7 do
				if IsBlank(nodedata.attr[i]) then break
				else str=str..','..nodedata.attr[i] end
	...
	taskbosswander={
		...
		default={["attr"]={"60","-96,96","112,144","16,32","8,16","MOVE_NORMAL","MOVE_X_TOWARDS_PLAYER"},
			["type"]="taskbosswander"},
		needancestor={'task','taskdefine','tasker'},
		forbidancestor={'bulletdefine','laserdefine','laserbentdefine','enemydefine'},
		allowchild={},
		totext=function(nodedata)
			local nf
			if IsBlank(nodedata.attr[1]) then nf='1' else nf=nodedata.attr[1] end
			if IsBlank(nodedata.attr[2]) then nodedata.attr[2]="-96,96" end
			if IsBlank(nodedata.attr[3]) then nodedata.attr[3]="112,144" end
			if IsBlank(nodedata.attr[4]) then nodedata.attr[4]="16,32" end
			if IsBlank(nodedata.attr[5]) then nodedata.attr[5]="8,16" end
			local mmode,dmode
			if IsBlank(nodedata.attr[6]) then mmode = "MOVE_NORMAL" else mmode = nodedata.attr[6] end
			if IsBlank(nodedata.attr[7]) then dmode = "MOVE_X_TOWARDS_PLAYER" else dmode = nodedata.attr[7] end
	...
	taskbosswander={
		...
		{'X Range','any',CheckExprOmit},
		{'Y Range','any',CheckExprOmit},
		{'Amplitude','any',CheckExprOmit},
		{'Movement Mode','movetomode',CheckExprOmit},
		{'Direction Mode','directmode',CheckExprOmit},
		disptype='boss wander',
		default={["attr"]={"30","-64,64","128,144","16,32","MOVE_NORMAL","MOVE_X_TOWARDS_PLAYER"},
			["type"]="taskbosswander"},
		...
		totext=function(nodedata)
			local nf
			if IsBlank(nodedata.attr[1]) then nf='1' else nf=nodedata.attr[1] end
			if IsBlank(nodedata.attr[2]) then nodedata.attr[2]="-160,160" end
			if IsBlank(nodedata.attr[3]) then nodedata.attr[3]="96,192" end
			if IsBlank(nodedata.attr[4]) then nodedata.attr[4]="48,64" end
			local mmode,dmode
			if IsBlank(nodedata.attr[5]) then mmode = "MOVE_NORMAL" else mmode = nodedata.attr[5] end
			if IsBlank(nodedata.attr[6]) then dmode = "MOVE_X_TOWARDS_PLAYER" else dmode = nodedata.attr[6] end
			return "boss wander "..nf.." frame(s), range of ("..nodedata.attr[2].."):("..nodedata.attr[3].."), "..
				"amplitude of "..nodedata.attr[4]..", "..nodedata.attr[5]..", "..nodedata.attr[6]
		end,
		tohead=function(nodedata)
			local nf
			if IsBlank(nodedata.attr[1]) then nf='1' else nf=nodedata.attr[1] end
			...
			if IsBlank(nodedata.attr[5]) then mmode = "MOVE_NORMAL" else mmode = nodedata.attr[5] end
			if IsBlank(nodedata.attr[6]) then dmode = "MOVE_X_TOWARDS_PLAYER" else dmode = nodedata.attr[6] end
			return string.format("task.MoveToPlayer(%s,%s,%s,%s,%s,%s)\n",nf,nodedata.attr[2],nodedata.attr[3],
				nodedata.attr[4],mmode,dmode)
		end,
	},]]
	--
	callbackfunc={
	...
	enemydefine={
		...
		tohead=function(nodedata) className=nodedata.attr[1] difficulty=string.match(nodedata.attr[1],'^.+:(.+)$') return string.format("_editor_class[%q]=Class(enemy)\n",className) end,
		tofoot=function(nodedata) difficulty=nil return '' end,
...
",className,p,nodedata.attr[2],nodedata.attr[3],nodedata.attr[8],nodedata.attr[9],nodedata.attr[10],nodedata.attr[4],nodedata.attr[5],nodedata.attr[6],nodedata.attr[7]) end,
		tofoot=function(nodedata) return "end\n" end,
	},
	enemycreate={
		...
		forbidparent={'root','folder'},
		allowchild={},
		totext=function(nodedata) return string.format("create enemy of type %q in (%s) with parameter %s",nodedata.attr[1],nodedata.attr[2],nodedata.attr[3]) end,
		tohead=function(nodedata)
			local p
		...
		check=function(nodedata)
			...
			if paramNumDict[class]~=CalcParamNum(nodedata.attr[3]) then return "number of parameter is incorrect" end
	...
	enemysimple={
		...
		totext=function(nodedata) return string.format("create enemy in (%s)",nodedata.attr[3]) end,
		tohead=function(nodedata)
	...
	laserdefine={
		...
		tohead=function(nodedata) className=nodedata.attr[1] difficulty=string.match(nodedata.attr[1],'^.+:(.+)$') return string.format("_editor_class[%q]=Class(laser)\n",className) end,
		tofoot=function(nodedata) difficulty=nil return '' end,
		check=function(nodedata) difficulty=string.match(nodedata.attr[1],'^.+:(.+)$') end,
		checkafter=function(nodedata) difficulty=nil end,
	},
...
",className,p,nodedata.attr[2],nodedata.attr[3],nodedata.attr[4],nodedata.attr[5],nodedata.attr[6],nodedata.attr[7],nodedata.attr[8]) end,
		tofoot=function(nodedata) return "end\n" end,
	},
	lasercreate={
		{'Type name','selecttype',CheckNonBlank},
	...
	lasergrow={
		...
		totext=function(nodedata) return string.format("grow in %s frame(s)",nodedata.attr[1]) end,
		tohead=function(nodedata)
	...
	laserchange={
		...
		totext=function(nodedata) return string.format("set %s's style to laser%s and color to %s",nodedata.attr[1],nodedata.attr[2],nodedata.attr[3]) end,
		tohead=function(nodedata)
			local ret
	...
	laserbentdefine={
		...
		allowparent={'root','folder'},
		allowchild={'callbackfunc'},
		totext=function(nodedata) return string.format("define bent laser type %q",nodedata.attr[1]) end,
		depth=0,
		...
		tohead=function(nodedata) className=nodedata.attr[1] difficulty=string.match(nodedata.attr[1],'^.+:(.+)$') return string.format("_editor_class[%q]=Class(laser_bent)\n",className) end,
		tofoot=function(nodedata) difficulty=nil return '' end,
		check=function(nodedata) difficulty=string.match(nodedata.attr[1],'^.+:(.+)$') end,
		checkafter=function(nodedata) difficulty=nil end,
	},
...
",className,p,nodedata.attr[2],nodedata.attr[3],nodedata.attr[4],nodedata.attr[5],nodedata.attr[6]) end,
		tofoot=function(nodedata) return "end\n" end,
	},
	laserbentcreate={
		{'Type name','selecttype',CheckNonBlank},
		...
		forbidparent={'root','folder'},
		allowchild={},
		totext=function(nodedata) return string.format("create bent laser of type %q with parameter %s",nodedata.attr[1],nodedata.attr[3]) end,
		tohead=function(nodedata)
			local p
			if IsBlank(nodedata.attr[3]) then p="_" else p=nodedata.attr[3] end
			return string.format("last=New(_editor_class[%q],%s,%s)\n",nodedata.fullclassname,nodedata.attr[2],p)
	...
	taskdefine={
		...
		allowparent={'root','folder'},
		--allowchild={'callbackfunc'},
		totext=function(nodedata) return string.format("define task %q (%s)",nodedata.attr[1],nodedata.attr[2]) end,
		--depth=0,
		watch='taskdefine',
		default=
		{
	...
	taskattach={
		...
		forbidparent={'root','folder'},
		allowchild={},
		totext=function(nodedata) return string.format("attach task %q to (%s) with parameter %s",nodedata.attr[1],nodedata.attr[3],nodedata.attr[2]) end,
		tohead=function(nodedata)
		...
		check=function(nodedata)
			...
			if paramNumDict[class]~=CalcParamNum(nodedata.attr[2]) then return "number of parameter is incorrect" end
			nodedata.fullclassname=class
		end,
	},
	--
	bulletdefine={
		...
		tohead=function(nodedata) className=nodedata.attr[1] difficulty=string.match(nodedata.attr[1],'^.+:(.+)$') return string.format("_editor_class[%q]=Class(bullet)\n",className) end,
		tofoot=function(nodedata) difficulty=nil return '' end,
		check=function(nodedata) difficulty=string.match(nodedata.attr[1],'^.+:(.+)$') end,
		checkafter=function(nodedata) difficulty=nil end,
	},
...
",className,p,nodedata.attr[2],nodedata.attr[3],nodedata.attr[4],nodedata.attr[5]) end,
	...
	bulletcreatestraight={
		...
		totext=function(nodedata)
			...
			if nodedata.attr[15]=='true' then ret=ret..", break through the screen" end
			if nodedata.attr[11]=='true' then ret=ret..", rebound when touching the wall" end
			return ret
		end,
		tohead=function(nodedata)
			if #nodedata.attr[10]==0 then nodedata.attr[10]='0' end
			if #nodedata.attr[12]==0 then nodedata.attr[12]='0' end
			if #nodedata.attr[13]==0 then nodedata.attr[13]='0' end
			if #nodedata.attr[14]==0 then nodedata.attr[14]='0' end
			local aca = nodedata.attr[13]
			if nodedata.attr[13]=='original' then aca = "'original'" end
	...
	bulletcreatestraightex={
		...
		totext=function(nodedata)
			...
			if nodedata.attr[15]=='true' then ret=ret..", rebound when touching the wall" end
			return ret
		end,
		tohead=function(nodedata)
			if #nodedata.attr[14]==0 then nodedata.attr[14]='0' end
	...
	objectdefine={
		...
		tohead=function(nodedata) className=nodedata.attr[1] difficulty=string.match(nodedata.attr[1],'^.+:(.+)$') return string.format("_editor_class[%q]=Class(_object)\n",className) end,
		tofoot=function(nodedata) difficulty=nil return '' end,
		check=function(nodedata) difficulty=string.match(nodedata.attr[1],'^.+:(.+)$') end,
		checkafter=function(nodedata) difficulty=nil end,
	},
...
",className,p,nodedata.attr[2],nodedata.attr[3],nodedata.attr[4],nodedata.attr[5],nodedata.attr[6],nodedata.attr[7],nodedata.attr[8],nodedata.attr[8],nodedata.attr[9]) end,
		tofoot=function(nodedata) return "end\n" end,
		--[[check=function(nodedata)
	...
	unitpreserve={
		...
		totext=function(nodedata) return string.format("preserve %s",nodedata.attr[1]) end,
		tohead=function(nodedata)
	...
	bossdefine={
		...
		tohead=function(nodedata)
			...
			if difficulty==nil or difficulty=='' then difficulty='All' end
			local scbg,_bg
			if IsBlank(nodedata.attr[10]) then
				_bg='nil'
			else _bg=nodedata.attr[10]
			end
...
_editor_class[%q].difficulty = %q\n"
	...
	bossspellcard={
		...
		tofoot=function(nodedata)
			...
			if IsBlank(nodedata.attr[1]) then
				...
				return a..string.format("table.insert(_editor_class[%q].cards,_tmp_sc)\ntable.insert(_sc_table,{%q,%q,_tmp_sc,#_editor_class[%q].cards,%s})\n",className,className,nodedata.attr[1],className,toboolean(nodedata.attr[10]))
			end
		end
	},
	bossspellcardex={
	...
	bosscast={
		...
		allowchild={},
		totext=function(nodedata) return string.format("play a cast animation in %s frame(s)",nodedata.attr[1]) end,
		tohead=function(nodedata) return string.format("boss.cast(self,%s)\n",nodedata.attr[1]) end,
	},
	bosssetui={
		...
		needancestor={'bossdefine','bossexdefine','bossspellcardex'},
		allowchild={},
		totext=function(nodedata) return "set boss ui display" end,
		tohead=function(nodedata) return string.format("boss.SetUIDisplay(self,%s,%s,%s,%s,%s)\n",nodedata.attr[1],nodedata.attr[2],nodedata.attr[3],nodedata.attr[4],nodedata.attr[5]) end,
	},
	bossshowaura={
...
self=self.ex\n"
		...
		tofoot=function(nodedata) difficulty=nil return 'self=_temp\nend\n' end,
		check=function(nodedata)
			if not IsBlank(nodedata.attr[5]) then
				local absfn=MakeFullPath(nodedata.attr[5])
				if not wx.wxFileName(absfn):FileExists() then return string.format("Resource file %q does not exist",absfn) end
	...
	bosspreparecards={
		...
		needancestor={'task','bossexdefine'},
		allowchild={},
		totext=function(nodedata)
			local a=0
			if not IsBlank(nodedata.attr[2]) then a=a+1 end
			if not IsBlank(nodedata.attr[3]) then a=a+1 end
			if not IsBlank(nodedata.attr[4]) then a=a+1 end
			if not IsBlank(nodedata.attr[5]) then a=a+1 end
			if not IsBlank(nodedata.attr[6]) then a=a+1 end
	...
	bosscastcard={
		...
		totext=function(nodedata)
			local a=0
			local b=''
			if IsBlank(nodedata.attr[1]) then a='next card' else a=nodedata.attr[1] end
			if nodedata.attr[2]=='true' then b='and finish ' end
			return "cast "..b..'"'..a..'"'
		end,
		tohead=function(nodedata)
			if IsBlank(nodedata.attr[1]) then
	...
	bossexplode={
		...
		totext=function(nodedata)
			local a=0
			if nodedata.attr[2]=='true' then return 'finish spell' end
			if nodedata.attr[1]=='true' then a='explode' else a='run away' end
			return a
		end,
		tohead=function(nodedata)
			if nodedata.attr[2]=='true' then
	...
	playsound={
		...
		tohead=function(nodedata)
			if IsBlank(nodedata.attr[4]) then nodedata.attr[4]=false end
	...
	setpace={
		...
		allowchild={},
		forbidparent={'root','folder'},
		totext=function(nodedata) return string.format("set pace start %s seconds, tick %s seconds",nodedata.attr[1],nodedata.attr[2]) end,
		tohead=function(nodedata) 
	...
	setworlds={
		...
		allowchild={'addworld'},
		forbidparent={'root','folder'},
		totext=function(nodedata) return string.format("set worlds") end,
		tohead=function(nodedata) 
	...
	addworld={
		...
		totext=function(nodedata) return string.format("World Size (%s * %s), left = %s, bottom = %s, mask = %s",nodedata.attr[1],nodedata.attr[2],nodedata.attr[4],nodedata.attr[5],nodedata.attr[6]) end,
		tohead=function(nodedata) 
	...
	loadtexture={
		...
		allowchild={},
		allowparent={'root','folder'},
		totext=function(nodedata) return string.format("load texture %q from %q",nodedata.attr[2],nodedata.attr[1]) end,
		tohead=function(nodedata) return string.format("LoadTexture('texture:'..%q,%q,%s)\n",nodedata.attr[2],wx.wxFileName(nodedata.attr[1]):GetFullName(),nodedata.attr[3]) end,
		check=function(nodedata)
			if checkImageName[nodedata.attr[2]]==nil then checkImageName[nodedata.attr[2]]=true
			else return string.format("Repeated resource name %q",nodedata.attr[2]) end
			local absfn=MakeFullPath(nodedata.attr[1])
	...
	loadmodel={
		...
		allowchild={},
		allowparent={'root','folder'},
		totext=function(nodedata) return string.format("load model %q from %q",nodedata.attr[2],nodedata.attr[1]) end,
		tohead=function(nodedata) return string.format("LoadModel('model:'..%q,%q)\n",nodedata.attr[2],wx.wxFileName(nodedata.attr[1]):GetFullName(),nodedata.attr[3]) end,
		check=function(nodedata)
			if checkImageName[nodedata.attr[2]]==nil then checkImageName[nodedata.attr[2]]=true
			else return string.format("Repeated resource name %q",nodedata.attr[2]) end
			local absfn=MakeFullPath(nodedata.attr[1])
	...
	loadimagegroup={
		...
		allowchild={},
		allowparent={'root','folder'},
		totext=function(nodedata) return string.format("load image group %q from %q",nodedata.attr[2],nodedata.attr[1]) end,
		tohead=function(nodedata) return string.format("_LoadImageGroupFromFile('image:'..%q,%q,%s,%s,%s,%s)\n",nodedata.attr[2],wx.wxFileName(nodedata.attr[1]):GetFullName(),nodedata.attr[3],nodedata.attr[4],nodedata.attr[5],nodedata.attr[6]) end,
		check=function(nodedata)
			if checkImageName[nodedata.attr[2]]==nil then checkImageName[nodedata.attr[2]]=true
			else return string.format("Repeated resource name %q",nodedata.attr[2]) end
			local absfn=MakeFullPath(nodedata.attr[1])
	...
	loadFX={
		...
		allowchild={},
		allowparent={'root','folder'},
		totext=function(nodedata) return string.format("load FX %q from %q",nodedata.attr[2],nodedata.attr[1]) end,
		tohead=function(nodedata) return string.format("LoadFX(%q,%q)\n",nodedata.attr[2],wx.wxFileName(nodedata.attr[1]):GetFullName()) end,
		check=function(nodedata)
			if checkImageName[nodedata.attr[2]]==nil then checkImageName[nodedata.attr[2]]=true
			else return string.format("Repeated resource name %q",nodedata.attr[2]) end
			local absfn=MakeFullPath(nodedata.attr[1])
	...
	bglayer={
		...
		tofoot=function(nodedata) return ")\n" end,
		check=function(nodedata)
--			if not watchDict.imageonly[nodedata.attr[1]] then return string.format('image %q does not exist',nodedata.attr[1]) end
	...
	bginit={
		allowparent={'bglayer'},
		--allowchild={"task"},
		totext=function(nodedata) return "on create" end,
		tohead=function(nodedata) return "function(self)\nself.task={}\n" end,
		tofoot=function(nodedata) return "end,\n" end,
	},
	bgframe={
		allowparent={'bglayer'},
		--allowchild={"task"},
		totext=function(nodedata) return "on frame" end,
		tohead=function(nodedata) return "function(self)\ntask.Do(self)\n" end,
		tofoot=function(nodedata) return "end,\n" end,
	},
	bgrender={
		allowparent={'bglayer'},
		--allowchild={"task"},
		totext=function(nodedata) return "on render" end,
		tohead=function(nodedata) return "function(self)\ntask.Do(self)\n" end,
		tofoot=function(nodedata) return "end\n" end,
	},
	patch={
	...
	shakescreen={
		...
		totext=function(nodedata) return "Shake screen "..nodedata.attr[1].." frames, amplitude of "..nodedata.attr[2] end,
		tohead=function(nodedata) return "misc.ShakeScreen("..nodedata.attr[1]..", "..nodedata.attr[2]..")\n" end
	},
	setfps={
		{'FPS','any',CheckExpr},
		disptype='Set FPS',
		...
		totext=function(nodedata) return "Set FPS of "..nodedata.attr[1] end,
		tohead=function(nodedata) return "SetFPS("..nodedata.attr[1]..")\n" end
	},
	highlight={
		{'Style','bulletstyle',CheckExpr},
		{'Color','color',CheckExpr},
		...
		totext=function(nodedata)
			local ret = 'turn '
			if nodedata.attr[3]=='true' then ret=ret..'on ' else ret=ret..'off ' end
	...
	archiexpand={
		...
		tohead=function(nodedata)
			...
			if a then center = string.format('{x=(%s),y=(%s)}',a,b) end
	...
	setaccel={
		...
		tohead=function(nodedata)
			...
			if rot == "original" then rot = "'original'" end
	...
	setfv={
		...
		tohead=function(nodedata)
			...
			if v  == "original" then v  = "'original'" end
			if vx == "original" then vx = "'original'" end
			if vy == "original" then vy = "'original'" end
	...
	rebounderdefine={
		...
		allowparent={'root','folder'},
		allowchild={'callbackfunc'},
		totext=function(nodedata) return string.format("define rebounder type %q",nodedata.attr[1]) end,
		depth=0,
		watch='rebounder',
		default=
		{
		...
		tohead=function(nodedata) className=string.match(nodedata.attr[1],'^(.+):.+$') or nodedata.attr[1] return string.format("_editor_class[%q]=Class(rebounder)\n",className) end,
		tofoot=function(nodedata) return '' end,
	},
	rebounderinit={
		{'Parameter list','any',CheckParam},
...
",className,p,nodedata.attr[2],nodedata.attr[3],nodedata.attr[4],nodedata.attr[5]) end,
	...
	reboundercreate={
		...
		forbidparent={'root','folder'},
		allowchild={},
		totext=function(nodedata) return string.format("create rebounder of type %q at (%s) with parameter %s",nodedata.attr[1],nodedata.attr[2],nodedata.attr[3]) end,
		tohead=function(nodedata)
			local p
			if IsBlank(nodedata.attr[3]) then p="_" else p=nodedata.attr[3] end
	...
	dropitem={
		...
		totext=function(nodedata)
			if nodedata.attr[2] == '0' then return "drop nothing" end
			local ret = "drop "
			if nodedata.attr[2] ~= '1' then ret=ret..nodedata.attr[2].." " end
	...
	_node_atp={
		...
		allowchild={},
		totext=function(nodedata) return "aim to player" end,
		tohead=function(nodedata) return "{'aim to player'},\n" end,
	},
	_node_g={
		{"Gravity","any",CheckExpr},
		{"Max Velocity","any",CheckExpr},
		...
		tohead=function(nodedata) return "{'gravity',"..table.concat(nodedata.attr, ", ").."},\n" end,
	},
	_node_accel={
		{"Acceleration","any",CheckExpr},
		{"Angle","any",CheckExpr},
		...
		tohead=function(nodedata) return "{'acceleration', "..table.concat(nodedata.attr, ", ").."},\n" end,
	},
	_node_stay={
		{"Time","any",CheckExpr},
		...
		tohead=function(nodedata) return "{'stay', "..nodedata.attr[1].."},\n" end,
	},
	_node_vf={
		{"Max Velocity","any",CheckExpr},
		...
		tohead=function(nodedata) return "{'velocity forbid', "..table.concat(nodedata.attr,", ").."},\n" end,
	},
	_node_pro={
		...
		tohead=function(nodedata) return "{'property', "..table.concat(nodedata.attr,", ").."},\n" end,
	},
	_node_dir={
		...
		tohead=function(nodedata) return "{'direction', "..table.concat(nodedata.attr,", ").."},\n" end,
	},
	_node_aa={
		...
		tohead=function(nodedata) return "{'jerk', "..table.concat(nodedata.attr,", ").."},\n" end,
	},
	_node_moveto={
		{"Position","any",CheckPos},
		{"Time","any",CheckExpr},
		...
		tohead=function(nodedata) return "{'moveto', "..table.concat(nodedata.attr,", ").."},\n" end,
	},
	_node_rot={
		...
		tohead=function(nodedata) return "{'rotation', "..table.concat(nodedata.attr,", ").."},\n" end,
	},
	_node_pe={
	...
	_node_toc={
		...
		allowparent={"shooter","lasershooter","bentlasershooter"},
		totext=function(nodedata) return "on condition that "..nodedata.attr[1] end,
		tohead=function(nodedata)
	...
	_node_tof={
		...
		allowparent={"shooter","lasershooter","bentlasershooter"},
		totext=function(nodedata) return "on frame" end,
		tohead=function(nodedata) return "{'task on frame', function(self) \n" end,
		tofoot=function(nodedata) return "end},\n" end,
	},
}
enumType={
