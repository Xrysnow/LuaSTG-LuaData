...
stage.group.DefStageFunc('SC Debugger@SC Debugger','init',function(self)
    ...
    task.New(self,function()
        ...
        local boss=_editor_class[_boss_class_name]
        local index=_boss_class_sc_index
        local cards={}
        if index then
            local cur_card=boss.cards[index]
            if cur_card[5] and boss.cards[index-1] then
                table.insert(cards,boss.cards[index-1])
            else
        ...
        local _boss_wait=true local _ref=New(boss,cards) last=_ref
