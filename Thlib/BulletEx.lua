...
function shooter:read(data)
	local switch = {
		["aim to player"] = function()
			...
			self.rot = self.rot + Angle(self, Player(self))
		end,
		...
		["property"] = function(indes, navi, bound, rebound, through)
			if indes then self.group = GROUP_INDES end
			self.navi = navi
			self.bound = bound
		...
		["moveto"] = function(x, y, time, cal, mode)
			...
			self.time = time
			task.New(self, function() task.MoveTo(ox, oy, time, mode) end)
		end,
		["rotation"] = function(radius, direct, navi)
			local w = self.v/radius
			if direct then direct = -1 else direct = 1 end
			self.acceleration = self.acceleration or {}
		...
		["task on condition"] = function(condition, _task)
			task.New(self, function()
				while true do
					if condition(self) then _task(self) end
					task._Wait(1)
				end
			end)
		end,
