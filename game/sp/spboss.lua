...
function lib.boss_default:init(x, y, slot, scbg)
	...
	self._blend, self._a, self._r, self._b, self._g = '', 255, 255, 255, 255
	lib.ResetBonus(self)
	self.is_finish = true
	self.hptimer = 0
	self.sc_left = 0
...
function lib.boss_default:frame()
	...
	if self.ex_card then self.ex_card.frame(self) end
	if self._card_system then self._card_system:frame(self) end
	if not(self.is_finish) and not(self.is_fake) then
		...
		if self.is_extra and lstg.player.nextspell > 0 then self.dmg_factor = 0 end
...
function lib.boss_default:kill()
	...
	if self.ex_card then self.ex_card.del(self) end
	if self._card_system then self._card_system:del(self) end
	...
	if self.is_final and not(self.no_killeff) and not(self.killed) then
		...
		if self._card_system then self._card_system:next(self) end
...
function lib.boss_default:turn_aura(open)
	_log(string.format("[spboss] Boss(%s) Aura Show Mode Changed", self))
	if open then
		if IsValid(self.aura) then self.aura.show = true end
	else
		if IsValid(self.aura) then self.aura.show = false end
	end
end
...
function lib.explode(boss)
	...
	task.New(boss, function()
		...
		boss.vx = 0.45 * cos(angle) boss.vy = 0.15 * sin(angle)
...
function lib.PopResult(boss, continue)
	...
	boss.sc_name = ''
	boss.is_finish = true
	if boss.is_final or not(continue) then Del(boss) end
	if continue then
		if boss.is_card then boss.sc_left = max(0, boss.sc_left - 1) end
		boss.DMG_factor = 1
		boss.timer = -1
		boss.hptimer = -1
		boss.card_timer = -1
...
function lib.change_scbg(boss, scbg, follow)
	if scbg == "nil" then
		if boss.bg and IsValid(boss.bg) then Del(boss.bg) boss.bg = nil end
		if IsValid(lstg.tmpvar.bg) then lstg.tmpvar.bg.hide = false end
	elseif scbg == "" then
		local t = 0
		local alpha = 0
		if boss.bg and IsValid(boss.bg) then alpha = boss.bg.alpha if follow then t = boss.bg.timer end Del(boss.bg) end
		boss.bg = New(spellcard_background)
		boss.bg.alpha = alpha
		if follow then for _ = 1, t do
...
function lib.set_init(boss, maxhp, bartype, dropitem, is_extra, is_final, ret, t1, t2, t3, is_sc)
	...
	boss.maxhp = maxhp boss.hp = maxhp
	_log(string.format("[spboss] Boss maxhp and hp set to %s", maxhp))
	boss.hpbartype = bartype
	local hpbartype
	if bartype == -1 then hpbartype = "Disabled" end
	if bartype == 0 then hpbartype = "Full" end
	if bartype == 1 then hpbartype = "Part_NonSpell" end
	if bartype == 2 then hpbartype = "Part_Spell" end
	...
	if ret then
		boss.timer = -1 boss.hptimer = -1 boss.card_timer = -1
...
function lib.SetStagePoint(boss, temp_hp, temp_t)
	if not(IsValid(lib.spcheckobj)) then New(lib.StagePointCheckobj) end
	if not(lib.StagePointList) then lib.StagePointList = {} end
	local hp = temp_hp or {}
	local t = temp_t or {}
	for i = 1, #hp do hp[i] = boss.maxhp - hp[i] end
	boss.sp_point = {}
	for i = 1, #hp do table.insert(boss.sp_point, (360 * (hp[i] / boss.maxhp) + 90)) end
...
function lib.StagePointCheck()
	...
	for k, v in pairs(lib.StagePointList) do
		...
		if IsValid(boss) then
			...
			for i = #hplist, 1, -1 do if hp <= hplist[i] then
				table.remove(plist, i) table.remove(hplist, i) table.remove(tlist, i)
				_log(string.format("[spboss] Boss(%s) Stage Point has been changed", boss))
			end end
			for i = #tlist, 1, -1 do if timer >= tlist[i] then
				table.remove(plist, i) table.remove(hplist, i) table.remove(tlist, i)
				_log(string.format("[spboss] Boss(%s) Stage Point has been changed", boss))
			end end
			boss.sp_point = plist
			if #plist ~= 0 then tmp[boss] = {boss, hplist, tlist} end
		end
	end
	lib.StagePointList = tmp
end
lib.StagePointCheckobj = Class(object)
function lib.StagePointCheckobj:init() lib.spcheckobj = boss end
function lib.StagePointCheckobj:frame() lib.StagePointCheck() end
...
function lib.CheckBonus(boss)
	if not(ex) or not(jstg and jstg.players[2]) then
...
function lib.ClearBonus(boss, type)
	...
	if boss._flag then boss._flag.score = nil end
end
...
function lib.EndChipBonus(boss, x, y)
	if boss.chip_bonus[1] and boss.chip_bonus[2] then
		...
		if boss.chip_bonus[1] then New(item_chip, x, y) end
		if boss.chip_bonus[2] then New(item_bombchip, x, y) end
	end
end
...
function lib.ResultSpell(info)
	if (not(info.time_sc) and not(info.timeout)) or (info.time_sc and info.timeout) then
		...
		if info.is_sc and info.timeout then PlaySound('fault', 1.0, 0) end
		if info.score then
...
function lib.ChangeImageGroup(boss, img, nRow, nCol, imgs, anis, a, b)
	if IsValid(boss) then
		...
		if not(img) or img == '' then
			for i = 1, 4 do boss['img'..i] = nil boss['ani'..i] = nil end
		else
			for i = 1, 4 do boss['img'..i] = nil boss['ani'..i] = nil end
			...
			for i = 1, nRow do boss['img'..i] = {} end
			for i = 2, nRow do boss['ani'..i] = imgs[i] - anis[i - 1] end
			for i = 1, nRow do for j = 1, imgs[i] do boss['img'..i][j] = 'anonymous:'..bossimg..i..j end end
		end
	end
end
...
function lib.connectdeath(...)
	...
	for i = 1, #l do
		l[i]._connect_death = {}
		for n = 1, #l do
			if n ~= i and l[i].is_spboss then table.insert(l[i]._connect_death, l[i]) end
		end
	end
end
...
function lib.aura:frame()
	if not(IsValid(self.boss)) then Del(self) end
	if self.show then
...
function lib.aura:render()
	if IsValid(self.boss) then
		for i = 1, 25 do SetImageState('boss_aura_3D'..i, 'mul+add', Color(self.boss.aura_alpha, 255, 255, 255)) end
...
function lib.sc_name:init(boss, name, slot)
	self.boss = boss
	self.name = name or ""
	if self.name == "" then RawDel(self) end
	self.slot = slot or 0
...
function lib.sc_name:frame()
	...
	if IsValid(self.boss) then
		if self.boss.is_spboss then
			...
			if self.score == 0 then self.score = nil end
		end
	end
end
...
function lib.card.add(id, name, t1, t2, t3, hp, dropitem, is_extra, before, start, finish, fake)
	...
	c.is_sc = not(name == '') c.name = name
	c.t1, c.t2, c.t3 = t1, t2, t3 c.hp = hp
	c.dropitem = dropitem c.is_extra = is_extra
	c.before = before c.start = start c.finish = finish
...
function lib.card.do_card(boss, id, bartype, is_final)
	...
	task.New(boss, function()
		...
		if c then
			...
			if c.finish then boss._task_finish = c.finish end
			if c.before then
				if not boss.task then boss.task = {} end
				local rt = coroutine.create(c.before())
				table.insert(boss.task, rt)
				local i = #boss.task
				while coroutine.status(boss.task[i]) ~= 'dead' do task.Wait() end
			end
			if c.start then
				lib.card.get_init(boss, c, bartype, is_final)
				if c.name ~= '' then lib.castcard(boss, c.name) end
...
function lib.card.prepare(boss, ...)
	...
	if #card_id_list>0 then
		...
		local c = 0
		for i = 1, #cards do
			if cards[i].is_sc then c = c + 1 end
		end
		boss.sc_left = c
...
function lib.card.dialog:WaitDialog()
	while self._in_dialog do task.Wait(1) end
	task.Wait(1)
end
...
function lib.ex_card:do_card_list(cards)
	...
	local c
	for i = 1, #curcards do
		if curcards[i].is_sc then c = c + 1 end
	end
	self.sc_left = c
...
function CardSystem:init(boss, cards)
	...
	for i = 1, #self.cards do
		if self.cards[i].is_combat then self.final = i end
		if self.cards[i].is_sc then self.sc_left = self.sc_left + 1 end
	end
end
---帧逻辑适配
---@param boss object @要执行符卡组的boss
