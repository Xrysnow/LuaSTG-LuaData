...
function stage_menu:init()
	...
	table.insert(menu_items,{'exit',function() jstg.SinglePlayer() menu.FlyIn(menu_title,'left') menu.FlyOut(menu_network,'right') end})
		...
		for i=1,60 do SetBGMVolume('menu',1-i/60) task.Wait() end
			...
			task.New(stage_menu,function()
				...
				elseif practice=='spell' then
					stage.IsSCpractice=true--add by OLC,for replay_saver
					RunSystem("on_game_start")
	...
	if stage.IsSCpractice then--add by OLC
