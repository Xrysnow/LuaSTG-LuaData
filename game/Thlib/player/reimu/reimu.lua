...
function reimu_sp_ef1:frame()
	local w=lstg.world--自适应值
	if BoxCheck(self,w.l,w.r,w.b,w.t) then self.inscreen=true end
	...
	if self.timer>150+self.tflag then
		...
		if self.inscreen then
			if self.x>w.r then self.x=w.r self.vx=0 self.vy=0 end
			if self.x<w.l then self.x=w.l self.vx=0 self.vy=0 end
			if self.y>w.t then self.y=w.t self.vx=0 self.vy=0 end
			if self.y<w.b then self.y=w.b self.vx=0 self.vy=0 end
		end
	end
